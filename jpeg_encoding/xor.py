...
def check_obeys_dna_constraints(
        strand, max_hp_length=4, max_gc_content=0.7):
    """
...
def check_dna_constraints_norec(strand, threshold = 0.1):
    return test_strand(strand) == 0.0


def get_valid_xor_seed_and_strand(strand, xor_seed_length=10, split_strand=False):
    ...
    while True:
        if split_strand:
            ...
            xor_seed = generate_random_xor_seed(xor_seed_length*4)
            bin_strand = convert_dna_to_binary_string(
                    strand)
            xored_bin_string = add_xor_mask(bin_strand, xor_seed)
            final_strand = convert_binary_string_to_dna(xored_bin_string)
            xor_seed_dna = convert_binary_string_to_dna(xor_seed)

        if check_dna_constraints_norec(final_strand):
            break
...
def get_bitstream_from_strand(strand, xor_seed, split_strands=False):
    ...
    if split_strands:
        ...
        bin_xor_seed = convert_dna_to_binary_string(xor_seed)
        fixed_bitstream = add_xor_mask(bin_strand, bin_xor_seed)
