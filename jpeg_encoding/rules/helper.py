...
def invalidPriority(packet, count : int, priority_chunks : int = None):
    ...
    used_packets = packet.used_packets
    threshold_prob = 0.8
    threshold_degree = 2
    ...
    if degree == 1:
        ...
        if (packet.used_packets[0] > priority_chunks):
            return True
    elif degree < threshold_degree:
        n_chunks_in_threshold = len(
            [i for i in used_packets if i <= priority_chunks])
        return (n_chunks_in_threshold / degree) < threshold_prob
